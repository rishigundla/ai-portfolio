...
| **Current Day** | Week 7 · CLOSED — Project 2 SHIPPED v1.0 (`v1.0-narrative-generator` tagged). **Awaiting confirmation to begin Project 3.** |
...
- [ ] **`flattenMarkdown` → `pptxgenjs` rich-text array** — the on-screen `_slide-preview.tsx` renders `**bold**` and `*italic*` via react-markdown faithfully, but the PPTX builder's `flattenMarkdown` helper strips emphasis to plain text. `pptxgenjs`'s rich-text array API (`addText([{text, options: {bold: true}}, ...])`) preserves per-run formatting but requires restructuring each slide builder to split prose into runs. **Effort**: ~half-day. Files: `lib/build-pptx.ts` (`addProseSlide` + `addTalkingPointsSlide` + the bullet rendering loop). Captured in case study's "What I'd do differently".
