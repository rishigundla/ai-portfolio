...
| **Current Day** | Week 5 · Day 6.1 — Project 1 Category A follow-ups CLOSED (RGB-triplet migration + visual regression baseline + fresh Lighthouse audit) |
...
- [ ] **`resumeBullet.text` precision** — current text says "Figma-exportable hi-fi wireframes" but the runtime is a modal explaining the future-state Figma MCP integration, not actual export. Tighter wording: "Figma-MCP-ready wireframes" or "wireframes designed for Figma MCP handoff". Recruiters read this and form expectations.
- [ ] **design-system-docs Dialog demo refresh** — the underlying Dialog primitive picked up the `animate-dialog-show` centering fix in W5.D4.4. The design-system-docs page that demonstrates Dialog likely propagates the fix automatically (it imports from the same package), but worth a Playwright probe to confirm the demo modal centers correctly.
- [ ] **Annotated git tag `v1.0-dashboard-factory`** — useful for commit archaeology and "what shipped in v1". Cosmetic but cheap.
