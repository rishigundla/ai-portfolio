...
| **Current Day** | Week 4 · Day 7 (Sun) — Dashboard uniqueness (per-domain layouts) |
| **Overall Progress** | 138 tasks of ~150 complete · **Production live at https://ai-portfolio-dashboard-factory.vercel.app** · Phase 0 ✓ · Week 1 ✓ · Week 2 ✓ · Week 3 ✓ · Week 4 Days 1-6 ✓ |
| **Status** | W4.D6 closed. Dashboard depth shipped in 3 phased commits. **Phase 1 (5cf3ab0) growth % deltas**: every measure-based KPI shows ↑/↓N% vs prior period. New `lib/period.ts` splits filtered rows by median date in the time column (late half = current, early half = prior). Returns null when either half has < 4 rows or all dates collapse to one — `Total Deals`, `Top X`, distinct counts skip the delta intentionally (period comparison isn't semantic for derived KPIs). No fixture mutation needed: existing data spans 9 mo (revops/marketing/supply), 6 mo (financial), 33 d (pulse), 4 yr (customer) — all enough for a clean split. **Phase 2 (b01163d) heatmap + scatter**: extended `DashboardChartData` union with two new variants. Slot-allocation: scatter when ≥2 measures (quadrant view of measure[0] vs measure[1], each row = one point, capped at 100 points), heatmap when time + dim (matrix of time-bucket × top-6 dim values, monthly bucketing for ≤18mo span / yearly bucketing otherwise). Heatmap is a hand-rolled CSS grid (Recharts has no heatmap primitive) with opacity-graded teal cells + legend bar; scatter uses Recharts `<ScatterChart>` with custom tooltip showing row id + both axis values. Result per dataset: revops/marketing/pulse/supply/customer get bar/line/scatter/heatmap, financial-complaints (1 measure) gets bar/line/donut/heatmap. **Phase 3 (d6e5c04) multi-select + between-dates filter**: two new compound sub-components in `@rishi/design-system` FilterBar. `FilterBar.MultiSelect` (popover trigger + cmdk Command check-list, "Enterprise +2" trigger truncation, full list via title for screen readers, clear-selection action). `FilterBar.DateRangePicker` (popover with 2× HTML5 date inputs constrained to dataset bounds via min/max, 3 preset chips with current state highlighted, Clear button). `DashboardFilters` shape change: `segmentValue: string` → `segmentValues: string[]` (empty = all), new optional `dateStart`/`dateEnd` ISO strings, `dateRange` enum gains `'custom'`. `applyFilters()` updated for set-membership segment + custom date window with open intervals. design-system-docs showcase page updated to demo both new primitives side-by-side with the originals. Verified live on production via Playwright: revops + pulse-telemetry both render correctly in dark + light, multi-select popover + date picker popover both open and interact correctly, zero console errors. Lighthouse held: home 98/100/100/100 (was 95, particle improvement), /dashboard/revops 79/100/100/100 (no W4.D6 regression, dashboard route's 79 is Recharts-driven not new-feature-driven). Build: 25/25 pages prerendered, /dashboard/[slug] 122 → 126 kB (+4 kB for new chart views), First Load 308 → 320 kB (+12 kB for popover + cmdk client chunks). |
| **Next Action** | Week 4 Day 7: Dashboard uniqueness (per-domain layouts). Each of the 6 dashboards uses a different curated chart mix tuned to its domain — RevOps: pipeline funnel + win-rate ring + region-channel matrix; Marketing: ROI quadrant scatter + funnel + channel CPA bars; Pulse: latency heatmap + health histogram + cluster size mix; Supply: fulfillment scorecard + supplier scatter + lead-time distribution; Financial: resolution-time distribution + state breakdown + dispute funnel; Customer: cohort retention matrix + LTV scatter + segment quadrant. Plus enhanced multi-tab drill-down dialog (rows + summary stats + distribution) and expandable rows in DrilldownTable. Cross-dataset QA at 1440px. |
| **Blockers** | None |
