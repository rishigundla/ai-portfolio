...
| **Current Day** | Week 12 · Day 2 - Sprint Intelligence README polish + portfolioCard refresh + root README live demos refresh shipped. Chart palette tokens added to the design system, 17 chart components swept from hardcoded hex literals to `var(--chart-*)` references. KPI tiles + heatmap + sprint gallery cards all theme cleanly in both light and dark. Project 3 case study + `v1.0-sprint-intelligence` git tag still pending. |
...
## Project 1 (Dashboard Factory) — SHIPPED v1.0 · 2026-05-14

**Tag**: `v1.0-dashboard-factory` (commit `e7bb441`) &nbsp;·&nbsp; **Live**: <https://ai-portfolio-dashboard-factory.vercel.app> &nbsp;·&nbsp; **Case study**: [`docs/case-studies/dashboard-factory.md`](./case-studies/dashboard-factory.md)

**Status**: Project 1 fully closed. Categories A (tech debt) and B (polish) both worked through after the initial close on W5.D6 — see W5.D6.1 + W5.D6.2 activity entries for the close-out details. Category C (future work) explicitly deferred to v2 per user scope call.

### Category A · Known tech debt (blocks "really done", not v1) — ✅ ALL CLOSED 2026-05-14

- [x] **`--color-accent` RGB-triplet migration** (commit `a5efc9c`, W5.D6.1) — dual-track pattern in `packages/design-system/src/tokens/colors.css`: keep hex tokens for direct SVG fill/stroke usage, add space-separated `-rgb` siblings for the 4 tokens used with opacity modifiers (accent, accent-light, base-900, status-completed). Tailwind config consumes the `-rgb` form via `rgb(var(--token-rgb) / <alpha-value>)`. Verified on production: `bg-accent/10` now resolves to `rgba(45, 212, 191, 0.1)` instead of `rgba(0,0,0,0)`. Every badge, pill, and tint surface across all 6 apps in the monorepo now renders correctly.
- [x] **Visual regression test suite baseline** (commit `d028a40`, W5.D6.1) — Playwright + Chromium added to `packages/design-system`, single Dialog test in `tests/visual/dialog.spec.ts` covers the W5.D4.4 centering bug class. Dual assertion: geometric centering check (`boundingBox` math with 2px tolerance) + screenshot diff (2% pixel-ratio tolerance). Baseline committed at `tests/visual/__snapshots__/dialog.spec.ts-dialog-open.png`. Run with `pnpm --filter @rishi/design-system test:visual`. CI integration not wired today (deferred to a Project 2+ scope).
- [x] **Fresh Lighthouse audit** (W5.D6.1) — `npx lighthouse --preset=desktop` against the 6 demo routes, scores well above the previous W5.D3-D4 numbers:

  | Route | Performance | Accessibility | Best Practices | SEO |
  |---|---|---|---|---|
  | `/` (home) | 100 | 100 | 100 | 100 |
  | `/datasets` | 96 | 98 | 100 | 100 |
  | `/generate/revops-sales` | 100 | 100 | 100 | 100 |
  | `/dashboard/revops-sales` | 96 | 100 | 100 | 100 |
  | `/wireframe` | 100 | 98 | 100 | 100 |
  | `/wireframe/manufacturing-throughput` | 97 | 98 | 100 | 100 |

  Every route ≥96 perf, ≥98 a11y, 100 best-practices, 100 SEO. Lowest score (96 perf) is well above the 90 "green" threshold. Raw JSON reports gitignored (`docs/lighthouse-reports/*.json`) — regenerate via `npx lighthouse <url> --preset=desktop --output=json --output-path=docs/lighthouse-reports/<route>.json`.

### Category B · Polish (nice-to-have, won't change the story) — ✅ ALL CLOSED 2026-05-14

- [x] **`resumeBullet.text` precision** (commit `e7bb441`, W5.D6.2) — tightened to "dataset-driven hi-fi wireframes designed for Figma MCP handoff" (was "Figma-exportable hi-fi wireframes" which implied the export works today). Recruiters now see accurate scope.
- [x] **design-system-docs Dialog demo refresh** (W5.D6.2, verified-only) — Playwright probe against the deployed design-system-docs at 1440×900 confirmed dialog centers pixel-perfectly: left=464, right=464, top=375, bottom=375 (horizontalDelta=0, verticalDelta=0). The W5.D4.4 fix propagated automatically through the monorepo dependency. No code change needed.
- [x] **Annotated git tag `v1.0-dashboard-factory`** (W5.D6.2, tag pushed to origin) — annotated tag at commit `e7bb441` with full release notes: what shipped in v1, Lighthouse scores, Cat A + Cat B closing summary, and the deferred Cat C list.

### Category C · Future work — ✅ DEFERRED TO v2 2026-05-14

User scope call: Cat C items are real-resource-cost themes, not Project-1-close polish. Doing them would commit days, money, or both — and the case study already framed them correctly as future work. Project 1 v1.0 is shipped and tagged; Cat C lives as the v2 backlog.

- Figma MCP integration wiring — architecture is ready (`WireframeLayout` is a typed handoff object); blocked on stable MCP access + API budget that doesn't suit a personal-portfolio project
- More datasets — HR analytics (cohort tables), security operations (log timelines), e-commerce (purchase funnels). Each stress-tests a chart kind the existing six don't. ~1 day per dataset of fixture authoring
- ML-driven KPI suggestions — replace hand-curated profiling fixtures with a model that consumes schema + sample rows and synthesizes the profile text. Architectural change, not a polish item
- Multi-user persistence — save dashboards, share via URL. Today everything is client-state in Zustand. Full-stack feature (Postgres + auth + API routes), multi-day
- Custom domain mapping for the live demo and case study — needs domain registration ($10-20/yr) + Vercel domain config

---

## Project 2 (Narrative Generator) — SHIPPED v1.0 · 2026-05-15

**Tag**: `v1.0-narrative-generator` &nbsp;·&nbsp; **Live**: <https://ai-portfolio-narrative-generator.vercel.app> &nbsp;·&nbsp; **Case study**: [`docs/case-studies/narrative-generator.md`](./case-studies/narrative-generator.md)

**Status**: Project 2 fully closed. Six dashboard fixtures + six narrative fixtures shipped, streaming readout panel live, server-rendered 7-slide deck preview with CDN-cached PPTX download endpoint. Three of four routes hit perfect Lighthouse 100/100/100/100. Case study at 1,570 words.

### Category A · Known tech debt (blocks "really done", not v1)

- [x] **`flattenMarkdown` → `pptxgenjs` rich-text array** (post v1.0 polish round, commits `6e20aa2` + `061ecdd` + `d926655`). Shipped. `lib/build-pptx.ts` now uses a `toRichText` helper plus a `parseInline` parser that converts a small subset of markdown (bold, italic, inline code) into the rich text array shape `pptxgenjs` accepts. Bold and italic emphasis survive end to end and match the on-screen preview. Three slides (`What moved`, `So what`, `Risks and caveats`) also moved from prose blocks to bullet lists at the same time, so the fidelity upgrade landed alongside a layout density upgrade.
- [ ] **Lighthouse-in-CI** — same gap as Project 1's Cat A #2 (visual regression). Project 2 inherits the issue: the W6.D6 a11y regression on `/dashboards` (contrast + heading-order) only got caught when a fresh audit ran 3 days after the regression shipped. A GitHub Action running `npx lighthouse` against preview URLs on every PR would catch structural a11y bugs (heading-order, contrast, focus-order) at the same step that catches build failures. Shared infrastructure for Projects 3-5; reasonable to land as a one-time setup before Project 3 kicks off.

### Category B · Polish (nice-to-have, won't change the story)

- [ ] **PPTX font embedding** — Space Grotesk + JetBrains Mono fall back to system fonts on machines without them. `pptxgenjs` supports font embedding via `pres.theme` + base64-encoded font files. Bloats the PPTX ~10× (from ~110 KB to ~1.1 MB) but lifts visual fidelity from "works" to "matches the design system pixel-for-pixel". Reasonable trade-off for a paid product; overkill for a portfolio demo unless someone shares the deck externally.
- [ ] **Open-Graph thumbnail for `/deck/[slug]`** — when someone shares a deck URL in Slack/Twitter/etc., an og:image preview would lift sharability. Could reuse the existing `/icon.tsx` pattern with `next/og`'s `ImageResponse`, generating one per slug. ~half-day.
- [ ] **Rename `_pptx-modal.tsx` → `_pptx-download-button.tsx`** — the file was a Radix Dialog modal stub in W6.D5; W7.D1 swapped it to an `<a href download>` anchor but kept the filename + component name (`PptxExportModal`) to avoid import churn. Now that the surrounding code is settled, the rename is a 3-touch change (file + import + component name). Cosmetic but cheap.
...
### 2026-05-17 · 🌞 W12.D1 - Sprint Intelligence light-mode theming pass
...
- **Next**: W13.D1 - Project 3 case study at `docs/case-studies/sprint-intelligence.md`. Three engineering moments: monorepo `rootDirectory` workaround, search-param-driven filter pattern, manifest enrichment pattern.

### 2026-05-17 · 🔎 W11.D7 - Sprint Intelligence filter-aware cross-sprint history and trend charts
...
- **Next**: W13.D1 - Project 3 case study at `docs/case-studies/sprint-intelligence.md`. Three engineering moments: monorepo `rootDirectory` workaround, search-param-driven filter pattern (now scoped across every dashboard surface and cross-sprint widget), manifest enrichment pattern.

### 2026-05-17 · 📆 W11.D6 - Sprint Intelligence per-engineer per-week scheduler, drop Ticket progress
...
- **Next**: W13.D1 - Project 3 case study at `docs/case-studies/sprint-intelligence.md`. Three engineering moments: the monorepo `rootDirectory` workaround, the search-param-driven filter pattern, and the manifest enrichment pattern.

### 2026-05-17 · 🔍 W11.D5 - Sprint Intelligence sequential engineer queue, team-view heatmap, Ticket progress rename
...
- **Next**: W13.D1 - Project 3 case study at `docs/case-studies/sprint-intelligence.md`. Three engineering moments: the monorepo `rootDirectory` workaround for the Vercel deploy, the search-param-driven filter pattern shared across every dashboard surface, and the manifest enrichment pattern that keeps cross-sprint widgets zero cost on First Load JS.

### 2026-05-17 · 🗓️ W11.D4 - Sprint Intelligence realistic workSchedule, sparse heatmap, Gantt run segments, trend chart numbers
...
- **Build clean, type check clean.** `/sprint/[id]` First Load JS unchanged at 50.4 kB. Redeployed to prod, all eight routes plus two filtered variants returned HTTP 200.
- **Next**: W13.D1 - Project 3 case study at `docs/case-studies/sprint-intelligence.md`. Three engineering moments: monorepo `rootDirectory` workaround, search-param-driven filter pattern (now shared across every dashboard surface — top filter feeds KPI strips, charts, deep dive, heatmap, Gantt), manifest enrichment pattern.

### 2026-05-17 · 📈 W11.D3 - Sprint Intelligence throughput redesign, deep dive merge, heatmap, Gantt
...
- **SprintTrendCharts bars rendered.** The `items-end h-40` collapse bug on `ClosedPerSprintChart` and `StoryPointsPerSprintChart` is fixed using the same `h-full` + `minHeight: 4px` pattern that fixed `PriorityBreakdownChart` in W11.D2. Both charts now show visible stacked columns per sprint.
...
- **Next**: W13.D1 - Project 3 case study at `docs/case-studies/sprint-intelligence.md`. Mirror the narrative-generator case study structure. Three engineering moments to cover: the monorepo `rootDirectory` workaround, the search-param-driven filter pattern (now shared across every surface after the engineer tab strip retirement), and the manifest enrichment pattern.

### 2026-05-17 · 🎨 W11.D2 - Sprint Intelligence polish: BI content, chart fixes, hover affordances
...
- **Next**: W13.D1 - Project 3 case study at `docs/case-studies/sprint-intelligence.md`. Mirror the narrative-generator case study structure. Three engineering moments to cover: the monorepo `rootDirectory` workaround for the Vercel deploy, the search param driven filter pattern shared with the `eng=N` drill down, and the manifest enrichment pattern that keeps cross sprint widgets zero cost on First Load JS.

### 2026-05-17 · 📊 W11.D1 - Sprint Intelligence per ticket charts, future sprint variant, redeploy
...
- **Next**: W13.D1 - Project 3 case study at `docs/case-studies/sprint-intelligence.md`. Mirror the narrative-generator case study structure. Three engineering moments to cover: the monorepo `rootDirectory` workaround for the Vercel deploy (W10.D5), the search param driven filter pattern shared with the `eng=N` drill down (W10.D1 + W10.D7), and the manifest enrichment pattern that keeps cross sprint widgets zero cost on First Load JS (W11.D1). Wire `caseStudyUrl` in `portfolio.meta.json` once the file lands.
...
- **Ticket schema expanded.** `TicketType` is now `bug | development | enhancement | deployment` (replaces story / bug / task / spike). Every ticket carries `eta: string | null` and `createdAt: string` so the W10.D7 KPI strip and the W11.D1 per ticket charts have data to read.
...
### 2026-05-18 · 📊 W9.D3 - Sprint Intelligence team KPI cards live
...
### 2026-05-15 · 🏁 W7.D5 - Project 2 SHIPPED v1.0 · `v1.0-narrative-generator` tagged
- **Final close gate.** Project 2 fully closed: production live, case study shipped, READMEs polished, portfolioCard aligned, Lighthouse 100/100/100/100 on 3 of 4 routes, PPTX export wired + verified, annotated tag pushed. **No Project 3 work has started without explicit confirmation.**
- **Project 2 — Pending Follow-ups section** added to the top-of-file section directory (between Project 1's section and "Recent Activity Log"). Mirrors Project 1's Cat A/B/C structure:
  - **Cat A (blocks "really done")**: `flattenMarkdown` → `pptxgenjs` rich-text array (~half-day, lifts PPTX emphasis fidelity from "stripped to plain text" to "matches the on-screen preview"); Lighthouse-in-CI as shared infrastructure for Projects 3-5 (catches structural a11y bugs at PR time, same gap as Project 1's Cat A #2).
  - **Cat B (polish)**: PPTX font embedding (bloats file 10× but lifts visual fidelity); Open-Graph thumbnail for `/deck/[slug]` (Slack sharing); rename `_pptx-modal.tsx` → `_pptx-download-button.tsx` (cosmetic since the W7.D1 modal-to-anchor swap).
  - **Cat C (future work, deferred by design)**: dynamic dashboards (user-uploaded CSV); ML-driven narrative generation (replace hand-curated fixtures with a model pipeline); multi-format export (PDF + Google Slides); custom domain mapping (same item as Project 1).
- **Phase Progress Overview** updated: Project 2 row flipped from "In Progress (W6.D1 scaffold shipped)" → "**✅ Shipped v1.0**" with the tag, case study, and live links — same shape as Project 1's row.
- **Status row collapsed**: was accumulating ~2,000 chars of W5-W6 history. Replaced with a clean "Project 2 closed across W6-W7 in 14 days" summary + one-line prior-milestone pointer; full history stays in the activity log.
- **`Current Phase` shifted** Phase 2 (Project 2) → "Phase 2 · CLOSED → Phase 3 (Sprint Intelligence) awaits".
- **`Current Day` shifted** W7.D4 → "Week 7 · CLOSED — Project 2 SHIPPED v1.0".
- **`Next Action`** describes the W8.D1 scope (scaffold `apps/sprint-intelligence/`, mirror narrative-generator's bootstrap pattern) and explicitly notes "Awaiting user confirmation before starting Project 3". Alternative path: knock out Project 2's Cat A items first as W8 starting work.
- **`Overall Progress` 172/172 → 210/210**: 14 task-IDs added across W6.D1 → W7.D5 + W7.D5 close-tasks (3 IDs).
- **Annotated tag `v1.0-narrative-generator`** pushed to origin with full release notes covering: production URL, case-study path, 14-day shipping arc, 4-route product surface, PPTX endpoint design, Lighthouse 100/100/100/100 on 3 routes, the three engineering moments from the case study, and the Cat A/B/C close-state.
- **What ships in Project 2 v1.0** (one-paragraph recap):
  - Production at <https://ai-portfolio-narrative-generator.vercel.app>, 24 prerendered routes
  - 6 dashboard fixtures + 6 narrative fixtures hand-curated across RevOps / Marketing / Ops / Finance / HR / CX
  - Streaming readout panel reusing `@rishi/ai-core/replayFixture` + `AiNarrativeBlock` from packages/ (zero code duplication with Project 1)
  - Server-rendered 7-slide deck preview themed to per-dashboard colorToken
  - CDN-cached PPTX download endpoint via `pptxgenjs` (~110 KB binaries baked at build time via `dynamic = 'force-static'`)
  - Lighthouse desktop ≥98 perf / ≥98 a11y / 100 best-practices / 100 SEO across 4 routes; 3 of 4 perfect 100/100/100/100
  - 1,570-word case study at [`docs/case-studies/narrative-generator.md`](./case-studies/narrative-generator.md)
- **What didn't change**: every Project 1 deliverable still live (`v1.0-dashboard-factory`, [case study](./case-studies/dashboard-factory.md)). Closing Project 2 didn't touch Project 1 code; only docs cross-references.

### 2026-05-15 · 📖 W7.D4 — Narrative Generator case study shipped
- **New artifact**: [`docs/case-studies/narrative-generator.md`](./case-studies/narrative-generator.md) — **1,570 words across 9 sections**, mirroring Project 1's case study structure exactly (same load-bearing skeleton so the two case studies read as one portfolio voice).
- **Sections**: Hero (live + source + stack) → The problem (BI analyst readout cycle, vendor-narrative-tools failure mode, dashboard-to-deck handoff loss) → The approach: three steps, one shared engine (pick → read → download, monorepo-shared primitives with Project 1) → Architecture (ASCII tree of `fixtures/` + `lib/` + `app/` with each module annotated, two file-pattern moves explained) → Key technical decisions (4 trade-offs: pptxgenjs over python-pptx, flattenMarkdown over rich-text array, modal-to-anchor swap for the 85× bundle win, `dynamic='force-static'` for CDN-cached PPTX) → Engineering moments worth telling (3 stories: Vercel CLI rootDirectory gap + REST API workaround, Lighthouse a11y regression caught + fixed in W6.D6, 14.2 kB → 167 B First Load JS reduction) → What I'd do differently (flattenMarkdown rich-text upgrade, hand-curated fixtures don't scale, Lighthouse-in-CI for future projects) → Future work (PPTX font embedding, dynamic dashboards, multi-format export) → Footer (cross-links to live, source, Project 1 case study, master plan).
- **Voice matches Project 1**: declarative third-person, real code references inline (`@rishi/ai-core/replayFixture`, `lib/build-pptx.ts`, `dynamic = 'force-static'`), trade-offs framed as decisions not apologies, three engineering moments anchored to actual W6/W7 activity-log entries.
- **`caseStudyUrl` wired** in `apps/narrative-generator/portfolio.meta.json` → `https://github.com/rishigundla/ai-portfolio/blob/main/docs/case-studies/narrative-generator.md`. The portfolio-site project will pick this up on its next pull.
- **`portfolioCard.impact` refined** to name the three engineering moments explicitly (Vercel CLI rootDirectory, Lighthouse a11y, 14.2 kB → 167 B) — recruiter-readable signal that the case study has substance, not just a description.
- **W7.D4 forward-references removed**: narrative-generator README "case study (W7.D4)" → "case study [link]"; "will land in W7.D4" → "are in the case study". Root README Project 2 row + live-demos list now links the real case-study path.
- **No code changes** in this commit. Build state unchanged.
- **Next**: W7.D5 — annotated git tag `v1.0-narrative-generator` at the final commit with full release notes (matching Project 1's `v1.0-dashboard-factory` tag pattern). Mark Project 2 SHIPPED in plan files. Categorize follow-ups (A/B/C) mirroring Project 1's pending-follow-ups section. After W7.D5, the natural pause point before W8 Project 3 kickoff.

### 2026-05-15 · 📝 W7.D3 — README polish + portfolio metadata finalization
- **3 files updated**: `apps/narrative-generator/README.md` rewritten from scratch, root `README.md` flipped Project 2 to Live, `apps/narrative-generator/portfolio.meta.json` portfolioCard + resumeBullet refined against shipped behavior.
- **`apps/narrative-generator/README.md` rewrite** (was W6.D2 scaffold-state, every route marked "placeholder", architecture marked "planned" — all stale after W6.D3 → W7.D2 shipped the real thing):
  - Header now reads "Production · v1.0 in flight" with case-study placeholder pointing at the W7.D4 destination
  - Three-step product pitch (Pick → Watch Claude write → Download deck)
  - Routes table lists all 5 routes (`/`, `/dashboards`, `/generate/[slug]`, `/deck/[slug]`, `/api/deck/[slug]`) with their actual sizes (167 B → 47.8 kB First Load JS) + Lighthouse scores (3 of 4 at 100/100/100/100)
  - ASCII architecture diagram showing the `fixtures/` + `lib/` + `app/` tree with each module's role annotated
  - Explicit "Shared with Project 1" vs "Unique to Project 2" callouts so the monorepo-investment story is structurally obvious
  - Deployment section documents the Vercel CLI rootDirectory gap + the REST API workaround as a recipe (recruiter-visible signal of "I actually deployed this")
- **Root `README.md`** updates:
  - Status line shifted from "Project 1 closing · 159 / 150 tasks" to "Project 2 v1.0 in flight · 201 / 172 tasks · Project 1 shipped v1.0, Project 2 case study lands W7.D4"
  - Live demos list adds Project 2 ([ai-portfolio-narrative-generator.vercel.app](https://ai-portfolio-narrative-generator.vercel.app))
  - Projects table Project 2 row flipped to "**Live** · case study W7.D4" with link to the app + the live URL
- **`portfolio.meta.json` portfolioCard refinements**:
  - `description`: tightened to "Pick a sample dashboard, watch Claude write the executive readout, download a themed 7-slide PPTX" — drops the vestigial "PDF" framing from the W6.D1 draft
  - `resumeBullet.text`: rewritten to name the actual shipped surface (5-section narrative structure, 7-slide PPTX via pptxgenjs, six business domains, shared streaming primitive with Project 1)
  - `portfolioCard.approach`: was "Two-step factory" → now "Three-step factory sharing its engine with Project 1" with the actual section markers (Headline / What moved / So what / Talking points / Risks + caveats) and the dynamic='force-static' CDN-cache detail
  - `portfolioCard.impact`: adds the Lighthouse numbers + the 167 B First Load on `/deck/[slug]` (pure server-rendered) as concrete signals; case study forward-reference now points at W7.D4
- **No code changes** in this commit — pure documentation polish. Build state unchanged (24 prerendered routes, Lighthouse 100/100/100/100 on 3 of 4 routes).
- **Next**: W7.D4 — write `docs/case-studies/narrative-generator.md` following Project 1's case study structure: problem framing, approach, architecture, key decisions with trade-offs, engineering moments worth telling (Vercel CLI rootDirectory gap, the 14.2 kB → 167 B bundle win, the flattenMarkdown trade-off), lessons learned, future work. Then W7.D5 the `v1.0-narrative-generator` tag.

### 2026-05-15 · 🔍 W7.D2 — PPTX visual QA + Lighthouse re-audit (no regression)
- **Verification-only day**, no code shipped. The W7.D1 PPTX export passes every automated check; the visual fidelity check stays as a user-side step (open in PowerPoint / Keynote / Google Slides).
- **All 6 PPTXs downloaded from production** (`/api/deck/<slug>`):

  | Slug | Size | Magic | Slides |
  |---|---|---|---|
  | `revops-q3-readout` | 110,820 B | `PK \x03 \x04` | 7 |
  | `marketing-q3-performance` | 109,918 B | `PK \x03 \x04` | 7 |
  | `ops-uptime-quality` | 111,663 B | `PK \x03 \x04` | 7 |
  | `finance-quarter-close` | 111,834 B | `PK \x03 \x04` | 7 |
  | `hr-engagement-attrition` | 114,260 B | `PK \x03 \x04` | 7 |
  | `cx-csat-nps` | 113,554 B | `PK \x03 \x04` | 7 |

  Every PPTX is a valid ZIP archive containing the expected `ppt/slides/slide{1..7}.xml`. Sizes cluster around 110-114 KB — narrative length is the dominant factor (HR + CX have slightly longer talking-point sections so their archives are bigger).
- **Slide XML content verification** (sampled across fixtures):
  - **Headline KPI values correct per fixture**: revops `$14.4M ↑12.4%`, marketing `$6.3M ↑18.6%`, ops `99.9% ↑0.1%` (formatted from 99.94 via `.toFixed(1)`), finance `$38.4M ↑14.6%`, hr `412 ↑8.4%`, cx `52 ↑8.3%`. Subtext callouts ("NRR-driven not new-logo-driven", "32 net adds · 38 hires", "Industry top quartile · 1,840 responses") all present.
  - **Narrative prose intact**: revops slide-3 "What moved" preserves the W27 → W37 ACV trajectory, the win-rate delta (38.2%), the AOV growth (4.8%). Em-dashes survived. XML entities (`&amp;`, `&apos;`) appear correctly encoded — PowerPoint renders them as `&` and `'`.
  - **Talking-point bullets**: revops slide-5 has all 5 bullets numbered 01-05 in the correct order, with the Polaris Defense / APAC partner-risk / AOV margin-led / GRR explanation / EMEA Direct callouts intact.
  - **Closing-recap 5-KPI strip**: finance slide-7 lists Q3 Revenue / Gross Margin / OpEx / Net Income / Monthly Burn, each with value + delta arrow + percent. Closing line "Questions? Detailed dashboard linked in the appendix." present.
- **flattenMarkdown audit**: **zero markdown leaks across all 6 decks**. Grepped every `slide*.xml` in every PPTX for literal `**` — 0 hits. The regex strip is doing its job; no `**bold**` text leaked into the slide bodies as visible asterisks.
- **Lighthouse re-audit** to confirm the W7.D1 modal-to-anchor swap didn't regress `/deck/[slug]`:

  | Route | Perf | A11y | BP | SEO |
  |---|---|---|---|---|
  | `/` | 99 | 98 | 100 | 100 |
  | `/dashboards` | 100 | 100 | 100 | 100 |
  | `/generate/revops-q3-readout` | 100 | 100 | 100 | 100 |
  | `/deck/revops-q3-readout` | 100 | 100 | 100 | 100 |

  `/deck/[slug]` held its perfect 100/100/100/100 — the 14.2 kB → 167 B First Load JS reduction from W7.D1 did NOT regress anything. The bundle win is invisible in the headline score (can't go above 100) but real in FCP/LCP timings. Home perf dropped 100 → 99 — within the documented run-to-run Lighthouse variance.
- **What stays user-side**: open the downloaded `.pptx` files in PowerPoint / Keynote / Google Slides and check (1) font rendering — Space Grotesk + JetBrains Mono fall back to system fonts if not installed; (2) text overflow on long-narrative slides; (3) accent color rendering. None of these are blocking — the automated content + Lighthouse profile is clean.
- **Next**: W7.D3 — README polish (root + `apps/narrative-generator/README.md`) reflecting the live URL + case study link + the 4-route product surface; finalize `portfolio.meta.json` `portfolioCard` strings against the shipped behavior. W7.D4 will be the case study, W7.D5 the `v1.0-narrative-generator` tag.

### 2026-05-15 · 📥 W7.D1 — PPTX export wired end-to-end via pptxgenjs
- **Commit `dd8b4e7`** — 8 files, +749 / −107 lines. The Download-PPTX modal stub from W6.D5 becomes a real download link: clicking it fetches `/api/deck/<slug>` and saves `<slug>.pptx` to disk, themed to the dashboard's `colorToken` with the same 7-slide structure as the on-screen preview.
- **`lib/build-pptx.ts` (~330 lines)** — server-side PPTX builder:
  - `buildPptxBuffer(dashboard, narrative)` returns a `nodebuffer` ready to send as a Response body
  - 7 slide builders mirror `_slide-preview.tsx` structurally: Title / Headline / What moved / So what / Talking points / Risks + caveats / Closing recap
  - Layout: `LAYOUT_WIDE` (13.333 × 7.5 in, 16:9), inches throughout
  - Typography: Space Grotesk for display + body, JetBrains Mono for labels — same as the design system
  - `HEX_BY_TOKEN` map (accent / purple / blue / amber / rose / teal / green → hex strings) since pptxgenjs can't resolve CSS variables at render time
  - `flattenMarkdown` helper strips `**bold**` + `*italic*` + `` `code` `` since pptxgenjs's basic text API doesn't render inline markdown. Acceptable for v1; W7.D2 polish could upgrade to pptxgenjs's rich-text array API for emphasis fidelity
  - Same `parseNarrativeSections` + `extractBullets` the on-screen preview uses — preview and download stay in lockstep
- **`app/api/deck/[slug]/route.ts`** — GET handler that loads fixtures, calls `buildPptxBuffer`, returns the buffer with `Content-Type: application/vnd.openxmlformats-officedocument.presentationml.presentation` and `Content-Disposition: attachment`. `runtime = 'nodejs'` (pptxgenjs needs Node Buffer + zip). `dynamic = 'force-static'` + `generateStaticParams` over 6 slugs — Next.js prerenders the response bodies at build time, downloads served from cache. `Cache-Control: immutable` since fixtures are git-versioned.
- **`lib/format-kpi.ts`** — extracted `formatKpiValue` from `_slide-preview.tsx` so the preview and the builder share the same formatter. Added `formatKpiDelta` + `HEX_BY_TOKEN` here too.
- **`_pptx-modal.tsx` rewritten** from a Radix Dialog future-state stub to a plain `<a href download>` anchor. Component name kept as `PptxExportModal` to avoid import churn; rename in a later cleanup. The page wire passes `slug` so the link resolves to `/api/deck/<slug>`.
- **Bundle win**: `/deck/[slug]` dropped from **14.2 kB → 167 B First Load JS**. Removing the Radix Dialog import eliminated the only client component on the deck page, making it pure server-rendered. The PPTX bytes ride in `/api/deck/[slug]` instead, served from cache.
- **Build**: 24 prerendered routes (was 18 — added 6 SSG `/api/deck/[slug]` entries). Vercel auto-deploys on push.
- **Next**: W7.D2 — visual QA of downloaded PPTX files (open in PowerPoint / Keynote / Google Slides, verify typography + colors + layout read correctly), Lighthouse re-audit confirming the modal removal didn't regress `/deck/[slug]` scores. After W7.D2: W7.D3 README polish, W7.D4 case study, W7.D5 `v1.0-narrative-generator` tag.

### 2026-05-15 · 🪛 W6.D6 — cross-viewport QA + Lighthouse + polish pass
- **Two commits**: cross-viewport sweep + Lighthouse audit surfaced 3 polish items; `65f0806` ships the fixes.
- **Cross-viewport Playwright sweep** at 1440 × 900 / 1280 × 800 / 768 × 1024 across all 4 route patterns (`/`, `/dashboards`, `/generate/<slug>`, `/deck/<slug>`):
  - No horizontal overflow at any viewport on any route.
  - `/deck/<slug>` slide layout holds at all widths — 7 slides at 1.78 aspect ratio (= 16:9 ✓), closing-recap 5-KPI strip fits its grid even at 768 tablet width (each KPI cell 110px, all 5 KPI values render in that space).
  - `/generate/<slug>` 5-step progress strip wraps cleanly at 1280; no layout break.
- **Fresh Lighthouse desktop audit** on 4 routes:

  | Route | Perf | A11y | BP | SEO |
  |---|---|---|---|---|
  | `/` | 100 | 98 | 96 | 100 |
  | `/dashboards` | 100 | **94** | 96 | 100 |
  | `/generate/revops-q3-readout` | 100 | 100 | 96 | 100 |
  | `/deck/revops-q3-readout` | 100 | 100 | 96 | 100 |

  Three findings worth fixing — all addressed in commit `65f0806`:
  1. **Favicon 404**: Lighthouse flagged `errors-in-console` on every route → BP dropped to 96. Cause: no `app/icon.tsx` + `app/apple-icon.tsx` in the scaffold. Fix: added both files mirroring dashboard-factory's pattern; 'NG' wordmark on the teal accent gradient.
  2. **Card-footer color contrast on `/dashboards`** (the a11y regression to 94): "5 KPIs · 3 charts · X rows" stat-row used `text-text-dim` (#4b5563) which gives 2.37:1 against `bg-surface` (#16161e). Below WCAG AA's 4.5:1 threshold. Fix: swap to `text-text-muted` (#94a3b8, lifted to slate-400 in W4.D2 for exactly this purpose); contrast lifts to 6.4:1.
  3. **Heading-order skip on `/dashboards`**: H1 → H3 on card titles (no H2). Fix: card titles changed `<h3>` → `<h2>` since cards are top-level gallery sections. Semantic improvement, not just a Lighthouse box-check.
- **Build after polish**: 18 prerendered routes (was 16 — added `/icon` + `/apple-icon` auto-routes from Next.js).
- **Re-audit after polish** (commit `65f0806` deployed):
  ...
  Wins: BP 96 → **100 on every route** (favicon fix did exactly what Lighthouse predicted). `/dashboards` a11y 94 → **100** (card-footer contrast + heading-order both addressed). Three routes hit a perfect 100/100/100/100. Home dropped 100 → 98 perf — likely run-to-run LCP variance from the new favicon route loading on first paint; not a real regression. Project 2 is now at or above Project 1's Lighthouse baseline.
- **Next**: W7.D1 — wire the actual PPTX export via `pptxgenjs`. The slide structure + `formatKpiValue` + `parseNarrativeSections` are all ready to plug in; the W7.D1 work is plumbing the server-side render of the same 7-slide sequence to a `.pptx` binary, applying design-system colors + typography to slide masters.

### 2026-05-15 · 🎞️ W6.D5 — /deck/[slug] preview wired end-to-end (7 themed slides)
- **Commit `d61b174`** — 6 files, +484 / −28 lines. The end-to-end story works on production: navigate `/dashboards` → click a card → `/generate/[slug]` (streaming readout) → click "Generate deck" → `/deck/[slug]` (7-slide preview).
- **7-slide deck sequence**, each themed by the dashboard's `colorToken`:
  1. **Title** — dashboard meta (icon, domain badge, title, tagline, period, audience)
  2. **Headline** — hero KPI (label, big-display value via `formatKpiValue`, delta, subtext)
  3. **What moved** — narrative section, rendered via react-markdown
  4. **So what** — narrative section
  5. **Talking points** — bullets extracted from the narrative, rendered with numbered prefixes
  6. **Risks + caveats** — narrative section
  7. **Closing recap** — 5-KPI strip + "Questions?" CTA
- Each slide is a **16:9 aspect-ratio card** with a slide-number badge in the corner and content padded for slide-like typography. Backgrounds use the dashboard's color-token gradient so manufacturing's accent-teal deck looks visually distinct from finance's green or HR's rose.
- **New `lib/parse-narrative.ts`** — tiny regex parser: `parseNarrativeSections` splits a narrative on `## ` headings into a `{ byHeading, ordered }` shape; `extractBullets` pulls `- ...` bullets from a section body. ~70 lines, no remark/unified dependency at module level. The W7.D1 PPTX builder will reuse the same parse.
- **New `_slide-preview.tsx`** — server component rendering the 7-slide sequence. Includes a `formatKpiValue` helper that handles all 7 KPI formats (currency / percent / count / rating / duration / ratio / score / categorical) with compact display ("$14.4M" not "$14,400,000.00").
- **New `_pptx-modal.tsx`** — "Download PPTX" CTA + Radix Dialog stub explaining W7.D1 future-state. Mirrors Project 1's Figma future-state modal pattern: cosmetic affordance signals architectural readiness; the binary export is a known-quantity follow-up.
- **`page.tsx` wired** with `getFullDashboard` + `getNarrativeFixture`. Both fixtures must exist or `notFound()`. Includes the modal in the header so users see the export affordance even before clicking through.
- **`package.json` adds `react-markdown` + `remark-gfm`** at the same versions as the design-system package (`^9.0.3` / `^4.0.0`). Needed because the slide renderer uses ReactMarkdown directly for prose sections + bullet inline formatting — same primitive Project 1 uses inside AiNarrativeBlock, just imported one level lower for slide-level rendering control.
- **Build**: `/deck/[slug]` grew from 169 B → **14.2 kB / 127 kB First Load JS**. Other routes unchanged. All 16 routes still prerender. Vercel auto-deploys on push.
- **Next**: W6.D6 — cross-viewport QA at 1440 / 1280 / 768 + fresh Lighthouse audit + the closing-recap KPI overflow polish (5 KPIs cramped on tablet width). After that, W7.D1 wires the actual PPTX export via `pptxgenjs`.

### 2026-05-15 · 🎙️ W6.D4 — 6 narrative fixtures + streaming panel wired end-to-end
- **9 new files**, +328 lines: 6 narrative JSON fixtures + 1 typed loader + 1 client component + 1 page-wiring update. Commit `80f7745`.
- **6 narrative fixtures** at `fixtures/narrative-generator/narratives/`, each ~3 KB and hand-curated in exec-readout voice (declarative, third-person, slide-ready). Structured around 5 markdown sections that map 1:1 to deck slides:
  1. `## Headline` — hero metric + delta in one punchy line
  2. `## What moved` — 2-3 sentences on the week-over-week story
  3. `## So what` — implications + recommended action
  4. `## Talking points` — 3-5 bullets ready for slide-by-slide expansion
  5. `## Risks + caveats` — what's NOT in the dashboard
- **Each narrative references its dashboard-fixture KPIs verbatim** so the streamed text stays grounded (e.g. revops narrative cites ACV $14.4M, win rate 38.2%, GRR 93.9%, Pipeline coverage 3.8x — same numbers as the dashboard JSON).
- **`lib/narratives.ts` loader** (~60 lines): static-imports the 6 JSONs (tree-shaken per route), exports `getNarrativeFixture(dashboardSlug)`, declares `NARRATIVE_SECTIONS` metadata + heading-marker constants (`HEADING_HEADLINE`, `HEADING_WHAT_MOVED`, etc.) the StreamingPanel + future deck renderer will consume.
- **`_streaming-panel.tsx` client component** (~200 lines): mirrors Project 1's streaming panel almost exactly. Imports `replayFixture` from `@rishi/ai-core` (zero code duplication — shared infrastructure win), uses `AiNarrativeBlock` from `@rishi/design-system/components` for the markdown + live cursor. 5-step progress strip lights up as each section heading appears in the streamed text. Cancel + Replay buttons, same UX as Project 1. "Generate deck" CTA after completion routes to `/deck/[slug]`.
- **`/generate/[slug]/page.tsx`** now pulls dashboard summary + narrative fixture, passes both to `<StreamingPanel>`. Both 404 on unknown slug. `generateStaticParams` still prerenders all 6 slugs.
- **Build**: 16 routes prerendered, `/generate/[slug]` grew from 169 B → 47.8 kB First Load JS (replayFixture + react-markdown + primitives). Matches Project 1's `/generate/[slug]` at 49 kB — same primitive chain.
- **Production verification via Playwright** at <https://ai-portfolio-narrative-generator.vercel.app/generate/marketing-q3-performance>:
  - "Claude is writing the readout..." indicator visible
  - Cancel button visible during streaming
  - 5-step progress strip rendering: `"✓Headline" / "2What moved" / "3So what" / "4Talking points" / "5Risks + caveats"` — first section already marked done when probed, second active, rest pending
  - AiNarrativeBlock rendering the streamed markdown
  - Per-slug page title via `generateMetadata` working: "Generating · Marketing Q3 Performance · Narrative Generator"
- **CDN-cache footnote**: production URL initially served the W6.D3 placeholder for ~60 seconds after the W6.D4 deploy went green. Waiting briefly resolved it. Worth flagging in the playbook — "Vercel deployed" ≠ "Vercel aliased + edge-propagated".
- **Next**: W6.D5 — author the deck-rendering primitives in `/deck/[slug]`. Use the dashboard fixture (`getFullDashboard`) + narrative fixture (`getNarrativeFixture`) to render: title slide, hero-metric slide, per-section slides matching the narrative structure. PPTX export wiring lands in W7.D1 via `pptxgenjs`.

### 2026-05-15 · 📊 W6.D3 — 6 dashboard fixtures + loaders + gallery wired
- **12 new files**, 1,208 lines added: 6 dashboard JSON fixtures + 1 manifest in `fixtures/narrative-generator/dashboards/`, plus 2 typed loaders (`lib/dashboards.ts` + `lib/full-dashboards.ts`) and 3 route page updates.
- **Fixtures authored** (each: 5 KPIs + 3 charts + 6-8 sample rows, ~120 lines):
  - `revops-q3-readout` — pipeline ACV, win rate, GRR, segment + channel breakdown
  - `marketing-q3-performance` — sourced pipeline, CPL, MQL→SQL, CAC payback, channel mix
  - `ops-uptime-quality` — uptime %, P95 latency, incident severity, MTTR, service breakdown
  - `finance-quarter-close` — revenue, gross margin, OpEx, net income, burn (margin lift narrative)
  - `hr-engagement-attrition` — headcount, eNPS, voluntary attrition by tenure, hiring funnel
  - `cx-csat-nps` — NPS by segment, CSAT trend, resolution time distribution, at-risk accounts
- **Each fixture is a FINISHED dashboard**, not a raw dataset — KPIs already computed, deltas already calculated, charts ready to render. The narrative-generator's job is to explain what these numbers MEAN, not to compute them. Different from Project 1's raw-dataset pattern by design.
- **Loaders mirror Project 1's split**:
  - `lib/dashboards.ts` — manifest-only (light, ~6 kB), feeds the gallery. Exports `DashboardSummary` / `ColorToken` / `getDashboardSummary` / `getAllDashboardSlugs` / `getDashboardIcon` / `getColorClasses`. Adds `green` to the color palette (finance dashboard).
  - `lib/full-dashboards.ts` — static-imports all 6 JSON fixtures (heavy), tree-shaken per route. Exports `DashboardFixture` / `KpiSpec` / `ChartSpec` / `getFullDashboard` / `getAllFullDashboards`. Three chart types declared (`line` | `bar` | `donut`) — Project 1's 7-kind dispatcher is overkill for narrative generation.
- **Route wiring**:
  - `/dashboards` now renders 6 real cards (3-column responsive grid) with icon + tagline + KPI/chart/row counts + click-through to `/generate/[slug]`.
  - `/generate/[slug]` adds `generateStaticParams` so all 6 slugs prerender. Pulls dashboard title + tagline from the manifest. Streaming panel placeholder still in place (W6.D6 lands the real one).
  - `/deck/[slug]` same `generateStaticParams` pattern. PPTX export lands W7.D1.
- **Build verification**: `pnpm --filter narrative-generator build` clean — **16 prerendered routes** (was 5 before W6.D3): 3 static (`/`, `/_not-found`, `/dashboards`) + 6 SSG `/deck/[slug]` + 6 SSG `/generate/[slug]` + 1 not-found. Both dynamic routes lifted from ƒ (dynamic) → ● (SSG). First Load JS still 105 kB.
- **Vercel auto-deploys** on push since GitHub is connected (W6.D2 wiring). Post-push verification in the W6.D3 commit message.
- **Next**: W6.D4 — author 6 narrative profiling fixtures (one per dashboard, ~3 KB each) at `fixtures/narrative-generator/narratives/`. Each narrative reads its dashboard fixture and outputs hero metric + week-over-week delta + contextual one-liner + 3-5 bullet structure for slides. Mirrors Project 1's profiling fixtures' voice.

### 2026-05-15 · 🚀 W6.D2 — Project 2 deployed to Vercel
- **Live**: <https://ai-portfolio-narrative-generator.vercel.app> (clean alias, HTTP 200, 884 ms cold). Aliased from internal URL `ai-portfolio-narrative-generator-b4qnw4yf6.vercel.app` to the convention name matching the layout.tsx fallback SITE_URL.
- **Deploy path was Vercel CLI** (not the dashboard UI). `npx vercel link --project ai-portfolio-narrative-generator --scope rishigundlas-projects` created + linked the project. `vercel git connect https://github.com/rishigundla/ai-portfolio` wired the GitHub integration so future pushes auto-deploy. The Vercel CLI doesn't expose Root Directory settings; set it via REST API (`PATCH /v9/projects/{id}` with body `{"rootDirectory":"apps/narrative-generator","framework":"nextjs"}`) using the locally-stored CLI token. After that, redeploying from the **monorepo root** (not from `apps/narrative-generator/` — Vercel would have doubled the path) succeeded cleanly.
- **Lighthouse desktop scores** (all 4 routes audited):

  | Route | Perf | A11y | BP | SEO |
  |---|---|---|---|---|
  | `/` (home) | 100 | 98 | 96 | 100 |
  | `/dashboards` | 100 | 100 | 96 | 100 |
  | `/generate/[slug]` (placeholder) | 100 | 100 | 96 | 90 |
  | `/deck/[slug]` (placeholder) | 100 | 100 | 96 | 90 |

  All routes ≥96 across the board, every route 100 perf. SEO 90 on the dynamic routes is expected — they don't have static metadata for arbitrary slugs yet; will lift to 100 once `generateStaticParams` lands with the 6 sample-dashboard slugs in W6.D3. BP 96 (vs Project 1's 100) is a Lighthouse rubric drift since W5.D6.1; worth a quick audit pass after W6.D3 if it persists.
- **`apps/narrative-generator/portfolio.meta.json`** updated: `liveUrl` and `deployedAt: "2026-05-15"` populated. `caseStudyUrl` and `loomUrl` left empty (case study lands in W7.D5; Loom likely skipped same as Project 1).
- **`apps/narrative-generator/README.md`** Live link updated to the production URL.
- **Next**: W6.D3 — author 6 sample dashboard fixtures spanning RevOps, Marketing, Ops, Finance, HR, CX. Pure data work; no UI changes. Each fixture is a JSON file with KPIs + chart specs + a few sample rows. After W6.D3 the `/dashboards` gallery in W6.D5 can render real cards.

### 2026-05-14 · 🚀 W6.D1 — Project 2 (Narrative Generator) scaffold shipped
- **User confirmed go-ahead** to begin Project 2 immediately after Project 1 v1.0 close. Scaffolded `apps/narrative-generator/` following the dashboard-factory bootstrap pattern. 15 files created. Build verified clean.
- **Config files** (8): `package.json` (Next.js 15, port 3003, design-system + ai-core workspace deps), `.gitignore`, `next.config.mjs` (transpilePackages for workspace TSX source), `postcss.config.js`, `tailwind.config.ts` (imports `@rishi/design-system/tailwind.config` preset), `tsconfig.json`, `next-env.d.ts`, `portfolio.meta.json` skeleton.
- **App shell** (3): `app/layout.tsx` matching dashboard-factory pattern (ParticleBackground + ThemeProvider + ThemeScript + Nav + footer + skip-link + full Next.js metadata/viewport), `app/globals.css` (design-system tokens import + base styles + `.section-container` class), `app/_components/Nav.tsx` ('NG' logo badge, Dashboards link, GitHub link, ThemeToggle).
- **Four route surfaces stubbed** (4):
  - `/` — home page with hero ("Dashboards out, stories back in") + 2-step preview cards
  - `/dashboards` — placeholder gallery, real cards land W6.D5
  - `/generate/[slug]` — placeholder streaming panel, real animation lands W6.D6
  - `/deck/[slug]` — placeholder slide preview, real rendering + PPTX export lands W7.D1
- **README**: routes table, planned architecture, dev/deploy steps, cross-link to Project 1's case study.
- **Build verified**:
  - `pnpm install` picked up the new workspace cleanly (no lockfile conflicts).
  - `pnpm --filter narrative-generator build` produced **5 routes (3 static + 2 dynamic), 105 kB First Load JS**.
  - Smaller bundle than dashboard-factory (which has Recharts + Zustand) — expected; this app's heavy deps (`pptxgenjs`, possibly `pdfjs-dist`) land in later days.
  - The dynamic routes (`/generate/[slug]`, `/deck/[slug]`) will move to SSG once `generateStaticParams` + slug list lands in W6.D3.
- **Weeks 6 + 7 in `weeksPlan` rewritten** to reflect Project 2 actually starting in Week 6 (was scheduled for Week 7 originally; Project 1 ran into Week 5 so everything shifted by 1 week). The old "Phase 1 Close" Week 6 content was deleted because Project 1 closed in W5 — the planned automation work was deferred to v2 alongside Cat C.
- **Phase Progress Overview** updated: Project 2 row shifted from "Awaiting kickoff confirmation" → "In Progress (W6.D1 scaffold shipped)".
- **Next**: W6.D2 — create the Vercel project pointing at `apps/narrative-generator`, verify deployed URL renders, run Lighthouse smoke audit, update `portfolio.meta.json` with `liveUrl` + `deployedAt`.

### 2026-05-14 · 🏁 Project 1 CLOSED · v1.0-dashboard-factory
- **Final close gate**. User scope call: Project 1 is done. Categories A and B both worked through after the initial close, Category C explicitly deferred to v2. **No Project 2 work starts without explicit confirmation.**
- **Phase Progress Overview** updated: Project 1 row now reads "**✅ Shipped v1.0**" with the tag, case study, and live links. Weeks-budget adjusted from "1-4" (the original plan) to "1-5" (actual — Week 5 was the close week).
- **Status row + Current Day** updated to lock the plan in "awaiting confirmation" state. `Next Action` describes the W6.D1 scope but explicitly notes it doesn't begin without go-ahead.
- **What v1.0 shipped** (one-paragraph summary):
  - Production live at <https://ai-portfolio-dashboard-factory.vercel.app>
  - 6 ad-hoc dataset modes + 6 dataset-driven wireframes, all sharing one chart engine (`lib/dashboard-builder.ts` `resolveChartPick` dispatcher across 7 chart kinds)
  - Streaming AI profiling animation that plays every visit, ~115s product moment
  - Interactive dashboard with 5-KPI strip, 4-chart grid, 3-tab drill-down dialog, PDF export
  - Export-to-Figma blurred-backdrop modal (future-state MCP integration story)
  - 28/28 pages prerendered, Lighthouse desktop ≥96 perf / ≥98 a11y / 100 best-practices / 100 SEO across 6 demo routes (3 routes hit perfect 100/100/100/100)
  - 1,294-word case study at [`docs/case-studies/dashboard-factory.md`](./case-studies/dashboard-factory.md) — problem framing, architecture, key decisions, engineering moments from the W5.D4.x fix saga, lessons, future work
  - Visual regression baseline (Playwright + Chromium) for the Dialog primitive in `packages/design-system/tests/visual/`
  - Annotated git tag `v1.0-dashboard-factory` at commit `e7bb441` with full release notes pushed to origin
- **What's deferred to v2** (Category C, explicitly): Figma MCP integration wiring (architecture ready, blocked on API budget), more datasets (HR / security ops / e-commerce), ML-driven KPI suggestions, multi-user persistence (Postgres + auth), custom domain mapping
- **What comes next is your call.** Project 2 (Narrative Generator) is the next planned project per the 14-week plan, but the plan is paused until you confirm.

### 2026-05-14 · W5.D6.2 — Project 1 Category B polish all closed
- **Cat B #1 — `resumeBullet.text` precision** (commit `e7bb441`). Tightened to "dataset-driven hi-fi wireframes designed for Figma MCP handoff" — accurate to what shipped, descriptive of the architecture that's actually ready (typed `WireframeLayout` handoff object), no implied promise of one-click export. Recruiters read this and form expectations; the new wording matches what visitors see.
- **Cat B #2 — design-system-docs Dialog demo refresh** (verified-only, no code change). Playwright probe at 1440×900 confirmed the Dialog demo centers pixel-perfectly: left=464, right=464, top=375, bottom=375, horizontalDelta=verticalDelta=0. The W5.D4.4 fix (`animate-dialog-show` keyframe) propagated to the docs app through the monorepo dependency on `@rishi/design-system/primitives`. Confirms the package-as-source-of-truth pattern is working.
- **Cat B #3 — annotated git tag `v1.0-dashboard-factory`** at commit `e7bb441`. Tag message includes: production URL, case-study path, README path, what's in v1 (datasets, wireframes, streaming, dashboard interactivity, PDF export, Figma modal), Lighthouse scores per route, Cat A + Cat B closing summary with commit refs, and the deferred Cat C list. Pushed to origin. Future-you doing commit archaeology will find this useful.

### 2026-05-14 · W5.D6.1 — Project 1 Category A follow-ups all closed
- **Three commits**, all of Category A done in a single session: `a5efc9c` (RGB-triplet migration), `d028a40` (visual regression baseline), and the present commit (Lighthouse audit + plan update).
- **Cat A #1 — `--color-accent` RGB-triplet migration**. Dual-track variable pattern: keep hex tokens for direct SVG usage (`fill="var(--color-accent)"` × 10 sites in `_dashboard-view.tsx`), add space-separated R G B siblings for the 4 tokens used with Tailwind opacity modifiers (accent, accent-light, base-900, status-completed). Tailwind config consumes `-rgb` siblings via `rgb(var(--token-rgb) / <alpha-value>)`. Verified on production: every static `bg-accent/X` (badges, pills, tint surfaces) across all 6 apps now resolves to teal-tinted instead of transparent. `hover:bg-accent/X` correctly stays transparent until hovered. Drift warning baked into colors.css header comment so hex + -rgb stay in sync.
- **Cat A #2 — Visual regression test suite baseline**. Playwright + Chromium added to `packages/design-system`. Single Dialog test at `tests/visual/dialog.spec.ts` covers the W5.D4.4 centering bug class with dual assertion: geometric centering check (`boundingBox` math vs viewport, 2px tolerance) AND screenshot diff (`toHaveScreenshot`, 2% pixel-ratio tolerance). Baseline PNG committed at `tests/visual/__snapshots__/`. Default base URL points at the deployed design-system-docs Vercel URL so tests run anywhere without a local dev server. `VISUAL_TEST_BASE_URL` env var overrides for local dev. `tests/README.md` documents the workflow + first-time setup. CI integration not wired today — would land as a GitHub Action on every PR in a future iteration.
- **Cat A #3 — Fresh Lighthouse audit**. `npx lighthouse --preset=desktop` against the 6 demo routes. All scores ≥96 perf, ≥98 a11y, 100 best-practices, 100 SEO. Three routes hit perfect 100/100/100/100 (home, /generate/revops-sales, /wireframe). Lowest: 96 perf on /datasets and /dashboard. Numbers improved over W5.D3-D4 baseline — the W5.D4.x fixes (CSS cleanup, removed figma-export page, dialog centering keyframe optimization) had a measurable positive impact on perf. Raw JSON reports gitignored (regeneratable); summary table lives in the Pending Follow-ups section.
- **Project 1 status now**: Category A (tech debt blocking "really done") fully closed. Category B (polish, ~1h total) and Category C (future work) still open per user's plan to work through them in order.

### 2026-05-14 · W5.D6 — Project 1 close: READMEs + portfolioCard + cross-viewport QA
- **`apps/dashboard-factory/README.md`** rewritten from scratch. The old version listed every route as "Pending" and still referenced the retired Executive/Operational/Exploratory templates. New version: production status, two-mode pitch matching the home page persona cards, current routes table (all Live, 28/28 pages prerendered), architecture-at-a-glance with code-file links (`lib/dashboard-builder.ts`, `lib/wireframe-engine.ts`), dev + deploy steps, cross-link to the case study and live demo.
- **Root `README.md`** updated: status line shifted from "Phase 1 · Week 1 complete (44 / 98 tasks · 44.9%)" to "Phase 1 · Project 1 (Dashboard Factory) closing · 159 / 150 tasks · case study shipped". Live demos section now lists both Project 0 and Project 1 with case-study link. Project 1 row in the projects table flipped from "Not started" → "**Live** · case study". The "each app will have its own README + meta + case study + Loom" sentence updated to past tense for Project 1 and points to the actual case study; the Loom reference dropped (consistent with W5.D5 reframe).
- **`apps/dashboard-factory/portfolio.meta.json`** `portfolioCard.problem/approach/impact` strings rewritten to match the case study's framing. Problem now lands on the two-audiences-two-timelines insight (BI ticket pain + Figma rebuild pain). Approach explicitly names "two modes, one shared chart engine". Impact points to the case study for architecture + engineering-moments depth instead of duplicating it inline.
- **Cross-viewport visual QA via Playwright** at 1440×900 (demo viewport), 1280×800 (smaller desktop), 768×1024 (tablet portrait):
  - `/` (home) at 1440: no horizontal overflow, both persona cards rendering, nav + footer present.
  - `/wireframe/manufacturing-throughput` at 1280: no overflow, all 4 chart headings present, funnel cone Pass(255px) / Rework(47px) / Scrap(38px) all centered (verified via `(parent − bar) / 2` math).
  - `/wireframe/logistics-fleet` at 768 (tablet portrait): no overflow, all 4 chart headings present, funnel cone Delivered(388px) / Delayed(39px) / In-Transit(26px) / Loaded(13px) all centered. Cone shape survives down to tablet width.
  - Zero console errors at any viewport.
- **Lighthouse re-audit deferred**: proper Lighthouse needs the Lighthouse CLI or PageSpeed Insights API, neither cleanly available from this session. Previous audit numbers from W5.D3-D4 are captured in the activity log (~95-100 perf / 98 a11y / 100 best-practices / 100 SEO across the routes). If fresh numbers are needed, run `npx lighthouse <url>` locally.
- **Project 1 is now functionally closed.** Open items not addressed here, marked for later: (a) the wider design-system fix for `--color-accent` RGB-triplet migration so `bg-accent/X` opacity modifiers work across all badges, (b) visual regression test suite that would have caught the transparent `bg-accent` bug on day one. Both are tracked in the case study's "What I'd do differently" section.
- **Next**: Week 6 — Project 2 (Narrative Generator) scaffold.

### 2026-05-14 · W5.D5 shipped — Dashboard Factory case study
- **Single commit** shipping `docs/case-studies/dashboard-factory.md` + `caseStudyUrl` wiring in `apps/dashboard-factory/portfolio.meta.json`.
- **1,294 words across 9 sections**: hero with stack + links, problem statement (the BI ticket + Figma rebuild double pain), approach (two modes, one engine), architecture (ASCII diagram + walkthrough of the chart engine + Zustand nav-guard + static fixtures), four key technical decisions with trade-offs (streaming profiling, data-driven wireframe engine pivot, funnel sort heuristic, Recharts choice), three engineering moments from the W5.D4.x fix saga (Tailwind opacity gotcha, funnel cone shape, Dialog centering), three "what I'd do differently" lessons, four future-work items (Figma MCP, more datasets, ML KPIs, multi-user), footer with cross-links.
- **Tone choice (per user)**: trimmed and accessible to recruiters + hiring managers as well as engineers. Technical depth in the architecture + decisions + fix-saga sections; the problem + approach sections frame for a non-engineering reader. Real code references inline (e.g., `buildFunnelChart`, `resolveChartPick`) without over-citing.
- **`caseStudyUrl` wired** in `apps/dashboard-factory/portfolio.meta.json` to `https://github.com/rishigundla/ai-portfolio/blob/main/docs/case-studies/dashboard-factory.md` — the portfolio-site project will pick this up the next time it pulls the meta file. No app rebuild needed.
- **What this commit doesn't do**: README polish, cross-viewport visual QA, final Lighthouse audit, finalize the `portfolioCard.problem/approach/impact` strings to align with the case study. Those land in W5.D6 (Project 1 close).

### 2026-05-14 · W5.D5 reframed (earlier today) — Loom out, case study + Project 1 close in
- **Scope call by user**: a Loom walkthrough doesn't add enough value to a personal portfolio project to justify the recording, editing, and YouTube-upload effort. Skip it entirely. Focus the remaining Project 1 capacity on a detailed case study + a clean close.
- **Reverted earlier in the same session**:
  - Deleted `docs/loom-voiceover-W5.D5.md` (the word-for-word narration artifact created in commit `e0460a5` a few minutes prior).
  - `apps/dashboard-factory/portfolio.meta.json` `loomUrl: ""` field left in place — harmless empty string, may be repurposed later, no need to remove the schema field.
  - The pre-flight Playwright sweep from `e0460a5` still has value as a record that every W5.D4.x fix renders cleanly at 1440×900 — that probe data remains in the activity log even though the Loom artifact is gone.
- **W5.D5 new direction**: write `docs/case-studies/dashboard-factory.md`. The `docs/case-studies/` folder was scaffolded back on 2026-04-24 awaiting content. Case study landing there means `apps/dashboard-factory/portfolio.meta.json`'s `caseStudyUrl` can point to the GitHub URL `https://github.com/rishigundla/ai-portfolio/blob/main/docs/case-studies/dashboard-factory.md` — discoverable from the portfolio site without needing an in-app route.
- **W5.D6 follow-on**: README polish (root + `apps/dashboard-factory/`), cross-viewport visual QA (1440 / 1280 / 768), final Lighthouse audit, finalize `portfolio.meta.json`. Tag Project 1 complete.
- **What didn't change**: every W5.D4.x production fix is still live (`1653b76` → `3cb3bb3`). This pivot is purely about reshaping what closes the project, not undoing the recent work.

### 2026-05-14 · W5.D5 prep (reverted) — voiceover script + pre-flight QA
- **Recording itself is a manual task** — what Claude can deliver for W5.D5 is the prep that makes the recording frictionless. This commit covers all of it.
- **Pre-flight Playwright sweep at 1440×900** across every route the script touches (`/`, `/datasets`, `/generate/revops-sales`, `/dashboard/revops-sales`, `/wireframe`, `/wireframe/manufacturing-throughput`):
  - Home: H1 reads "Dashboards in minutes, not days.", new "6 dataset-driven wireframes" copy present, stale "Executive, Operational, Exploratory" copy absent, both persona badges accent-styled.
  - `/datasets`: 6 cards linking to `/generate/<slug>`.
  - `/generate/revops-sales`: streaming live — "Claude is analyzing…" indicator + Cancel button + 4-step progress visible (confirming Fix 1's always-stream behavior).
  - `/dashboard/revops-sales`: 5-stage deal funnel auto-sorted descending (Closed Won 28 → Negotiation 6 → Proposal 3 → Closed Lost 2 → Qualification 1), 4 charts, KPI values rendering ($14.4M ACV, 93.9% GRR, 40 deals, Enterprise top segment).
  - `/wireframe`: 6 cards in the right order.
  - `/wireframe/manufacturing-throughput`: funnel cone Pass(318px) → Rework(59px@left=130) → Scrap(47px@left=135), all centered, all solid teal `rgb(45,212,191)`. No density selector. Export-to-Figma is a `<button>` (not `<Link>`).
  - **Zero console errors across the sweep.**
- **New artifact** `docs/loom-voiceover-W5.D5.md` — word-for-word narration timed to the 10 beats of the refreshed beat sheet. Target ~115s at ~149 wpm; 286 words total. Each beat includes a `[bracketed]` screen-action cue so the recording stays in sync with the voiceover. Includes pre-recording checklist, recording flow, and post steps.
- **`apps/dashboard-factory/portfolio.meta.json` content fix**: `portfolioCard.approach` referenced "pick a layout template" — stale since W5.D3 retired the Executive/Operational/Exploratory templates. Updated to "pick a dataset-driven wireframe, each layout is generated from the dataset schema by the same chart engine that powers the ad-hoc dashboard…" The `loomUrl` field already exists as an empty string; nothing to scaffold there — just edit and paste the YouTube URL after upload.
- **Hand-off**: when you're ready, follow the pre-recording checklist in the new artifact (key step: `localStorage.clear()` so /generate streams fresh), record, trim, upload to YouTube unlisted, paste the URL into `portfolio.meta.json` `loomUrl`, commit.

### 2026-05-10 · W5.D4.4 followup — Dialog modal centered on viewport
- **Single feat commit** (`3cb3bb3`): 2 files in the design-system package, +16 / −1 lines.
- **Reported issue**: user reported the Export-to-Figma popup not centered. Playwright `getBoundingClientRect` probe at 929×985 viewport showed the 448×464 dialog rendering at `left=465 right=17 top=493 bottom=29` — bottom-right quadrant.
- **Root cause**: `DialogContent` uses the canonical centering pattern `fixed left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%]`. But `data-[state=open]:animate-slide-up` runs the `slideUp` keyframes (`from {transform: translateY(30px)} to {transform: translateY(0)}`) with `forwards` fill mode — the final keyframe's `translate(0,0)` overwrites the inline centering translate. Result: top-left of the dialog at viewport center → dialog appears in the bottom-right quadrant.
- **Why this didn't break Popover / Select / Tooltip / Toast** (which also use `animate-slide-up`): those primitives don't use the `translate-50%` centering trick. Radix's positioning engine handles them, or they sit in fixed corners. Only Dialog hits the conflict.
- **Fix**: add a Dialog-specific keyframe `dialogContentShow` in `packages/design-system/tailwind.config.ts` with composite transform that preserves both axes' centering translate while still animating opacity + 8px Y slide:
  ```
  from { opacity: 0; transform: translate(-50%, calc(-50% + 8px)); }
  to   { opacity: 1; transform: translate(-50%, -50%); }
  ```
  New animation alias `dialog-show` (250ms ease-out forwards — slightly faster than the generic 600ms slide-up since modal entrances feel sluggish at that duration). `DialogContent` swaps `animate-slide-up` → `animate-dialog-show`.
- **Verification**: Playwright probe at 1920×1013 viewport now reports dialog at `left=736 right=736 top=275 bottom=275` — `horizontalDelta=0`, `verticalDelta=0`. Pixel-perfect centering.
- **Affects every Dialog in the app**: Export-to-Figma modal on `/wireframe/[slug]`, drill-down dialog on `/dashboard/[slug]` charts, any future Dialog uses. The dialog now appears centered immediately on open with a subtle slide-up entrance.

### 2026-05-09 · W5.D4.3 followup — funnel chart actually looks like a funnel
- **Single feat commit** (`9dace7b`): 2 files, +19 / −9 lines. Affects every funnel in every dashboard (ad-hoc + all 6 wireframes).
- **Reported issue**: user shared a screenshot of the Logistics Fleet "Fulfillment lifecycle" funnel showing Loaded(1) → In-Transit(2) → Delivered(30) → Delayed(3) — bars left-aligned, in the schema's lifecycle order. The numbers zigzagged (1 → 2 → 30 → 3, not monotonic) and the bars left-aligned at the start of the row, so the chart visually read as a horizontal bar chart, not a funnel.
- **Two changes, both app-wide**:
  - **`lib/dashboard-builder.ts` `buildFunnelChart`** — when the `dim.values`-ordered stages aren't monotonically non-increasing, fall back to sorting by value descending. The schema-declared sequence still wins when it produces a real funnel shape (Pass=27 / Rework=5 / Scrap=4 keeps semantic order); when it doesn't (status-snapshot data where the lifecycle order is meaningless), the chart sorts to produce the canonical narrowing shape. Each stage's `pct` reads as "% of stage 1" with descending order — the conventional funnel KPI.
  - **`FunnelChartView`** — center-align bars instead of left-align. The bar is now a flex item inside `justify-center`, so a 100% bar fills the row, a 50% bar centers within the row taking up half-width on either side, etc. This is the canonical funnel look (Mixpanel, Amplitude): biggest stage on top spanning the row, narrower stages centered below, the cone shape emerging from the layout itself. Tooltip text updated from "% of max" → "% of stage 1" since the ordering now guarantees that semantic. `min-w-[2px]` ensures the bar is visible even when stage value rounds to <1% width.
- **Affects every funnel**: ad-hoc `/dashboard` (revops Closed-Won funnel, marketing impressions→clicks→conversions, etc.) and every wireframe (`/wireframe/manufacturing-throughput` Pass→Rework→Scrap, `/wireframe/real-estate-pipeline` Active→Pending→Sold→Expired→Withdrawn, `/wireframe/healthcare-operations` Scheduled→Walk-in→Urgent→Emergency, `/wireframe/education-enrollment` Active→Completed→Withdrawn→Dropped, `/wireframe/saas-subscriptions` Trial→Active→At-Risk→Churned, `/wireframe/logistics-fleet` Loaded→In-Transit→Delivered→Delayed).
- **Trade-off acknowledged**: the descending-sort fallback breaks lifecycle ordering when the schema's natural order doesn't produce a funnel shape. For status-snapshot data this is desirable (the order had no semantic value to begin with). For genuine sequences that happen to violate monotonicity (e.g. an early-stage SaaS funnel with more Active than Trial), the chart visually deprioritizes the stage flow in favor of magnitude. Both readings are defensible; the funnel-shape reading wins because the chart is *labeled* a funnel.
- **Verification**: build clean (28/28 pages, no route size deltas). Production verification on the Vercel deploy follows.

### 2026-05-09 · W5.D4.2 followup — funnel bars actually visible
- **Single feat commit** (`82fa09e`): 1 file, 1 line of substantive change.
- **Reported issue**: user shared a screenshot of the Logistics Fleet wireframe's "Fulfillment lifecycle" funnel rendering as a black gap between the stage labels and the value column — the bars themselves invisible. Bug present on every wireframe with a funnel.
- **Diagnosis** via Playwright `getComputedStyle` probe:
  - Each bar `<button>` had its width set correctly (e.g. Delivered=549px, Loaded=18px) — geometry was fine
  - But `backgroundColor` resolved to `rgba(0,0,0,0)` — fully transparent → looked black against the dark card surface
  - Probe of all `bg-accent/10` and `bg-accent/30` badges across the app showed **the same transparent value** — the bug is app-wide, not funnel-specific
- **Root cause**: `--color-accent` is defined as a hex literal `#2dd4bf` in `packages/design-system/src/tokens/colors.css`, and the design-system Tailwind config maps `accent → 'var(--color-accent)'`. Tailwind's `bg-accent/75` opacity modifier expands to `background-color: rgb(var(--color-accent) / 0.75)` which becomes the invalid `rgb(#2dd4bf / 0.75)` — browsers drop it.
- **Why this only surfaced now**: dac4622 (W5.D4.1 Fix 4) removed the `mix-blend-screen` value overlay that was previously masking the bug — the dark text was bleeding into the visible card surface, making the bars look "darker but populated". With the overlay gone, the raw transparent rectangles became obvious.
- **Localized fix**: change funnel bar from `bg-accent/75` → `bg-accent` (solid teal) in `apps/dashboard-factory/app/dashboard/[slug]/_dashboard-view.tsx` `FunnelChartView`. Hover state changes from `hover:bg-accent` → `hover:bg-accent-light` so the disabled→hover transition is still visually distinct.
- **Verification**: Playwright `getComputedStyle` probe across `/wireframe/logistics-fleet` and `/wireframe/manufacturing-throughput` confirms `backgroundColor: rgb(45, 212, 191)` on every funnel bar, widths still proportional to value.
- **Wider issue not addressed here**: every `bg-accent/10` and `bg-accent/30` badge in the app is also transparent — the canonical fix is migrating `--color-accent` to space-separated RGB (`45 212 191`) and updating the Tailwind config to `'rgb(var(--color-accent) / <alpha-value>)'`. That's a cross-cutting design-system refactor that needs visual QA across all accent-tinted UI; tracking it as a separate piece of work rather than expanding scope here.

### 2026-05-09 · W5.D4.1 hotfix — 5 production-feedback UX fixes
- **Goal**: address 5 specific UX issues reported from production verification — the kind of quality-bar feedback that lands after a feature looks "done" but isn't yet polished.
- **5 sequential feat commits** (`1653b76` → `65794ee` → `5dd5a5d` → `dac4622` → `674f02b`), one per fix, plus this docs commit. Net repo delta **−440 lines** (mostly from deleting the 295-line figma-export page).
- **Fix 1 — Always play profiling animation** (`1653b76`):
  - `apps/dashboard-factory/app/generate/[slug]/_streaming-panel.tsx`: removed the Zustand `profilingComplete[slug]` short-circuit that was skipping the streaming text on return visits.
  - The streaming animation IS the headline product moment ("watch Claude profile it") — suppressing it on revisits defeated the demo.
  - Kept the `markProfilingComplete()` write because `/dashboard/[slug]`'s navigation guard still gates access by it. Decoupled UI suppression from access gating.
- **Fix 2 — Home persona-card style parity** (`65794ee`):
  - `apps/dashboard-factory/app/page.tsx`: switched "For Developers" `badgeAccent="muted"` → `"accent"` so both home cards present with equal visual weight.
  - Also refreshed the description (was stale post-W5.D3 — referenced the deleted Executive / Operational / Exploratory templates). New copy mentions the 6 dataset-driven wireframes spanning manufacturing, real estate, healthcare, education, SaaS, and logistics.
- **Fix 3 — Drop wireframe density selector** (`5dd5a5d` for engine + globals; UI removal rides with `674f02b`):
  - `apps/dashboard-factory/lib/wireframe-engine.ts`: removed `WireframeDensity` type, `WIREFRAME_DENSITIES` exported array, `density` parameter on `buildWireframeLayout()`, and the `density === 'sparse'` branching for KPI count.
  - `apps/dashboard-factory/app/globals.css`: deleted the `[data-density="sparse"]` and `[data-density="dense"]` rule blocks (originally added in W5.D3 commit `d2343a2`).
  - Every wireframe now renders one canonical layout: 5-KPI strip + 4-chart 2x2 grid (formerly "Balanced"). Sparse and Dense modes were over-built scope for what the user values.
- **Fix 4 — Funnel value labels readable at any width** (`dac4622`):
  - `apps/dashboard-factory/app/dashboard/[slug]/_dashboard-view.tsx` `FunnelChartView`: replaced 3-cell row with 4-cell row.
  - **Old**: `[label w-28][bar flex-1 with overlay value using mix-blend-screen text-base-900][percent w-12]` — clever but fragile blend hack that failed when the bar was too narrow to contain the text, leaving dark numbers floating on the dark canvas (nearly invisible on Healthcare's "Visit acuity ladder", Manufacturing's stage funnel).
  - **New**: `[label w-28][bar flex-1][value w-14 right]` `[percent w-12 right]` — value cell uses `text-text-primary font-mono font-semibold` on the dark surface, same readability as the percent column.
  - Trade-off: lose the "value-on-bar" aesthetic; gain guaranteed legibility at any bar width. Fix applies to all funnels (ad-hoc + wireframe modes).
- **Fix 5 — Export-to-Figma modal replaces dedicated page** (`674f02b`):
  - `apps/dashboard-factory/app/wireframe/_components/WireframeView.tsx`: CTA becomes a `<DialogTrigger>` instead of a `<Link>`. New `<ExportToFigmaModal>` uses the design-system Dialog primitive whose overlay is already styled `bg-base-900/80 backdrop-blur-sm` — the blurred backdrop is built-in.
  - Modal body: ~6-line narrative covering what the button would do (one-click MCP push to Figma), why it isn't wired today (Figma desktop + ongoing API spend that isn't justified for a personal-portfolio project on free Vercel hosting), and the architectural readiness (typed `WireframeLayout` ready for MCP handoff).
  - **Deleted** `apps/dashboard-factory/app/wireframe/figma-export/page.tsx` (295 lines). Empty parent directory removed. Verified zero references via Grep before deletion.
  - Build: 28 of 28 pages prerendered (was 29 — −1 for figma-export). Route stats unchanged.
- **Verification**:
  - `pnpm --filter dashboard-factory build` clean — 28/28 pages.
  - `npx tsc --noEmit` clean.
  - Vercel deploy live at 20:01 IST. Production checks via `curl`: `/wireframe/figma-export` now 404s, home shows new "6 dataset-driven wireframes" copy, `/wireframe/[slug]` no longer renders Sparse/Balanced/Dense radiogroup labels.
- **Next**: continue Week 5 Day 5 — Loom video walkthrough.
