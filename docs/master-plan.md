...
| **Current Day** | Week 5 · Day 6 — Project 1 CLOSED (case study + READMEs + portfolioCard + cross-viewport QA) |
...
- [ ] **`--color-accent` RGB-triplet migration** — every `bg-accent/X` opacity utility in the app resolves to transparent because Tailwind's `<alpha-value>` placeholder requires R G B triplets, not hex literals. Funnel got a localized fix in W5.D4.2 (solid `bg-accent`); badges across the app (nav pills, persona-card chips, home dataset-tags, etc.) still suffer. Surfaces aren't broken visually because the dark base-900 surface is close enough to the intended muted tint that the human eye doesn't notice — but it IS wrong CSS. **Effort**: ~half-day. Files: `packages/design-system/src/tokens/colors.css` + `packages/design-system/tailwind.config.ts` + visual sweep across all `bg-accent/*` consumers. Captured in case study's "What I'd do differently".
- [ ] **Visual regression test suite** — would have caught the transparent `bg-accent` on day one. Playwright screenshot tests committed to git, run on every PR. Should live in the design-system package since the primitives are the right test boundary. **Effort**: ~half-day for a minimal `dialog + button + KpiCard` baseline; ~day for full coverage across primitives. **Tooling**: Playwright's built-in screenshot assertions are enough; no need for Storybook + Chromatic unless we want PR-comment visual diffs.
- [ ] **Fresh Lighthouse audit numbers** — last captured numbers (95-100 perf / 98 a11y / 100 best-practices / 100 SEO) are from W5.D3-D4 (~3 weeks ago). Run `npx lighthouse <url> --view --preset=desktop` against `/`, `/datasets`, `/generate/revops-sales`, `/dashboard/revops-sales`, `/wireframe`, `/wireframe/manufacturing-throughput`. Paste numbers into a new activity entry. **Effort**: ~30 minutes. Not blocking — the recent W5.D6 cross-viewport QA confirmed zero overflow + zero console errors, which are the harder signals to surface.
